    ...
    public class CodeGeneratorApp
    {
        ...
        public string ApiCreate(BaseModel baseInfo, List<DbTableModel> dbTableList, Dictionary<string, CodeFormCompontModel> compontMap, ColModel colData)
        {
            try
            {
                ...
                sb.Append("namespace cx.Application.WebApi\r\n");
                sb.Append("{\r\n");
                sb.Append(NotesCreate(baseInfo));
                ...
                sb.Append("            : base(\"/cx/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "\")\r\n");
                sb.Append("        {\r\n");
        ...
        public string AppIndexJSCreate(BaseModel baseInfo, List<DbTableModel> dbTableList, Dictionary<string, CodeFormCompontModel> compontMap, ColModel colData, QueryModel queryData)
        {
            try
            {
                ...
                sb.Append("                cx.nav.go({ path: '"+ baseInfo.outputArea + "/" + baseInfo.name + "/form', title: '新增', type: 'right' });\r\n");
                sb.Append("            });\r\n");
                sb.Append("        },\r\n");
                ...
                sb.Append("            }\r\n");
                sb.Append("            cx.httpget(config.webapi + 'cx/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/pagelist', _postParam, (data) => {\r\n");
                ...
                sb.Append("            cx.nav.go({ path: '" + baseInfo.outputArea + "/" + baseInfo.name + "/form', title: '详情', type: 'right', param: { keyValue: item." + mainPkey + " } });\r\n");
                sb.Append("        },\r\n");
                sb.Append("        btnClick: function (item, $item, $page) {// 左滑按钮点击事件\r\n");
                sb.Append("            cx.layer.confirm('确定要删除该笔数据吗？', function (_index) {\r\n");
                sb.Append("                if (_index === '1') {\r\n");
                sb.Append("                    cx.layer.loading(true, '正在删除该笔数据');\r\n");
                sb.Append("                    cx.httppost(config.webapi + 'cx/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/delete', item."+ mainPkey + " , (data) => {\r\n");
                sb.Append("                        if (data) {// 删除数据成功\r\n");
                sb.Append("                            page.grid.reload();\r\n");
                sb.Append("                        }\r\n");
                sb.Append("                        cx.layer.loading(false);\r\n");
                sb.Append("                    });\r\n");
                sb.Append("                }\r\n");
        ...
        public string AppFormJsCreate(BaseModel baseInfo, List<DbTableModel> dbTableList, List<CodeFormTabModel> formData, Dictionary<string, CodeFormCompontModel> compontMap)
        {
            try
            {
                ...
                sb.Append("                cx.layer.confirm('确定要退出当前编辑？', function (_index) {\r\n");
                sb.Append("                    if (_index === '1') {\r\n");
                sb.Append("                        if (keyValue) {// 如果是编辑状态\r\n");
                sb.Append("                            cx.formblur();\r\n");
                ...
                sb.Append("                                    if (_index === '1') {\r\n");
                sb.Append("                                        cx.layer.loading(true, '正在删除该笔数据');\r\n");
                sb.Append("                                        cx.httppost(config.webapi + 'cx/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/delete', keyValue, (data) => {\r\n");
                sb.Append("                                            cx.layer.loading(false);\r\n");
                sb.Append("                                            if (data) {// 删除数据成功\r\n");
                sb.Append("                                                cx.nav.closeCurrent();\r\n");
                sb.Append("                                                var prepage = cx.nav.getpage('"+ baseInfo.outputArea + "/" + baseInfo.name + "');\r\n");
                ...
                sb.Append("                cx.layer.loading(true, '正在提交数据');\r\n");
                sb.Append("                cx.httppost(config.webapi + 'cx/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/save', _postData, (data) => {\r\n");
                sb.Append("                    cx.layer.loading(false);\r\n");
                sb.Append("                    if (data) {// 表单数据保存成功\r\n");
                sb.Append("                        if (keyValue) {\r\n");
                sb.Append("                            cx.layer.toast('保存数据成功!');\r\n");
                sb.Append("                            cx.formblur();\r\n");
                sb.Append("                            $header.find('.lr-form-header-cancel').hide();\r\n");
                sb.Append("                            $header.find('.lr-form-header-submit').hide();\r\n");
                sb.Append("                            $header.find('.lr-form-header-btnlist').show();\r\n");
                ...
                sb.Append("                $header.find('.lr-form-header-btnlist').show();\r\n");
                sb.Append("                // 获取表单数据\r\n");
                sb.Append("                cx.layer.loading(true, '获取表单数据');\r\n");
                sb.Append("                cx.httpget(config.webapi + 'cx/adms/" + baseInfo.outputArea + "/" + baseInfo.name + "/form', keyValue, (data) => {\r\n");
                sb.Append("                    if (data) {\r\n");
                sb.Append("                        for (var id in data) {\r\n");
                    ...
                    foreach (var compont in tab.componts)
                    {
                        switch (compont.type)
                        {
                            ...
                            case "datetimerange":
                                ...
                                sb.Append("                if (st && et) {\r\n");
                                sb.Append("                    var diff = cx.date.parse(st).DateDiff('d', et) + 1;\r\n");
                            ...
                            case "encode":
                                sb.Append("            if (!keyValue) {\r\n");
                                sb.Append("                cx.getRuleCode('"+ compont.rulecode + "', function (data) {\r\n");
                            ...
                            case "currentInfo":
                                ...
                                {
                                    sb.Append("            if (!keyValue) {\r\n");
                                    sb.Append("            $page.find('#" + compont.field + "').val(cx.date.format(new Date(), 'yyyy-MM-dd hh:mm:ss'));\r\n");
                                    sb.Append("            }\r\n");
                                }
                                else {
                                    sb.Append("            $page.find('#" + compont.field + "').lrselect({\r\n");
                                    ...
                                    {
                                        case "company":
                                            sb.Append("            $page.find('#" + compont.field + "').lrselectSet(cx.storage.get('userinfo').baseinfo.companyId);\r\n");
                                            break;
                                        case "department":
                                            sb.Append("            $page.find('#" + compont.field + "').lrselectSet(cx.storage.get('userinfo').baseinfo.departmentId);\r\n");
                                            break;
                                        case "user":
                                            sb.Append("            $page.find('#" + compont.field + "').lrselectSet(cx.storage.get('userinfo').baseinfo.userId);\r\n");
                                            break;
                                    }
                                    sb.Append("            }\r\n");
                                }
