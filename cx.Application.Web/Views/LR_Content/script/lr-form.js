...
(function ($, cx) {
            ...
            if ($obj.length == 0 && value != null) {
                ...
                switch (type) {
                    ...
                    case "lrGirdSelect":
                        ...
                        break;
                    case "datepicker":
                        $obj.val(cx.formatDate(value, 'yyyy-MM-dd'));
                        break;
                    case "lr-Uploader":
    ...
    $.lrSetForm = function (url, callback) {
        cx.loading(true, '正在获取数据');
        cx.httpAsyncGet(url, function (res) {
            cx.loading(false);
    ...
    $.lrExistField = function (keyValue, controlId, url, param) {
        ...
        $.extend(data, param);
        cx.httpAsync('GET', url, data, function (data) {
            if (data == false) {
