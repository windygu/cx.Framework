/*
 * 版 本 cx-ADMS V7.0.0 力软 敏捷 开发框架(http://www.cx.cn)
...
(function ($, cx) {
    ...
    var page = {
        init: function () {
            ...
            if ($('body').hasClass('IE') || $('body').hasClass('InternetExplorer')) {
                ...
                Pace.on('done', function () {
                    $('#lr_loadbg').fadeOut();
                    Pace.options.target = '#cxpacenone';
                });
            }
            // 通知栏插件初始化设置
            toastr.options = {
            ...
            // 打开首页模板
            cx.frameTab.open({ F_ModuleId: '0', F_Icon: 'fa fa-desktop', F_FullName: '我的桌面', F_UrlAddress: '/Home/AdminDesktop' }, true);
            cx.clientdata.init(function () {
                page.userInit();
                // 初始页面特例
                bootstrap($, cx);
                if ($('body').hasClass('IE') || $('body').hasClass('InternetExplorer')) {
        ...
        loginout: function () { // 安全退出
            cx.layerConfirm("注：您确定要安全退出本次登录吗？", function (r) {
                if (r) {
                    cx.loading(true, '退出系统中...');
                    cx.httpAsyncPost($.rootUrl + '/Login/OutLogin', {}, function (data) {
        ...
        clearredis: function () {
            cx.layerConfirm("注：您确定要清空全部后台缓存数据吗？", function (r) {
                if (r) {
                    cx.loading(true, '清理缓存数据中...');
                    cx.httpAsyncPost($.rootUrl + '/Home/ClearRedis', {}, function (data) {
                        window.location.href = $.rootUrl + "/Login/Index";
                    });
                }
            });
        ...
        openUserCenter: function () {
            // 打开个人中心
            cx.frameTab.open({ F_ModuleId: '1', F_Icon: 'fa fa-user', F_FullName: '个人中心', F_UrlAddress: '/UserCenter/Index' });
        },
