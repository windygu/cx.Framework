...
(function ($, cx) {
    ...
    $.lrtree = {
                    ...
                    if ($this.hasClass('lr-tree-node-expanded')) {
                        ...
                        if (!node.complete) {
                            if (!node._loading) {
                                ...
                                cx.httpAsync('GET', dfop.url, { parentId: node.id }, function (data) {
                                    if (!!data) {
        ...
        search: function (keyword, data) {
            ...
            $.each(data, function (i, row) {
                ...
                if (!cx.validator.isNotNull(keyword).code || row.text.indexOf(keyword) != -1) {
    ...
    $.fn.lrtree = function (settings) {
        var dfop = {
            ...
            data: [],
            isAllExpand: false,
            cbiconpath: top.$.rootUrl + '/Content/images/cxtree/',
            // 点击事件（节点信息）,节点$对象
            nodeClick: false,
            // 选中事件（节点信息）,节点$对象
            nodeCheck: false
        ...
        if (dfop.id == null || dfop.id == "") {
            dfop.id = "cxtree" + new Date().getTime();
    ...
    $.fn.lrtreeSet = function (name,op) {
        ...
        switch (name) {
            ...
            case 'search':
                ...
                if (cx.validator.isNotNull(op.keyword).code) {
                    dfop._isSearch = true;
                }
