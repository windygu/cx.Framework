...
(function ($, cx) {
    ...
    var httpGet = function (url, param, callback, loadmsg) {
        cx.loading(true, loadmsg || '正在获取数据');
        cx.httpAsync('GET', url, param, function (res) {
            cx.loading(false);
            callback(res);
        });
    };
    var httpPost = function (url, param, callback, loadmsg) {
        cx.loading(true, loadmsg || '正在获取数据');
        cx.httpAsync('Post', url, param, function (data) {
            cx.loading(false);
            callback(data);
        });
    };
    ...
    function getLoginInfo() {
        var req = {
            token: top.$.cookie('cx_ADMS_V7_Token'),
            loginMark: top.$.cookie('cx_ADMS_V7_Mark'),
        };
            ...
            httpGet(api.bootstraper, req, function (res) {
                if (res != null) {
                    if (res.status == 1) {
                        ...
                        cx.alert.error(res.desc);
                        op.callback(false);
                    }
                }
                else {
                    cx.alert.error('获取流程信息失败!');
                    op.callback(false);
                }
            }, '正在获取流程信息...');
        },
