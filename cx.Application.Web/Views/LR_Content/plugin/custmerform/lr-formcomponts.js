...
(function ($, cx) {
    ...
    $.lrFormComponents = {
        ...
        radio: {//单选框
            ...
            renderTable: function (compont, $row) {//使用表单的时候渲染成table
                ...
                if (compont.dataSource == '0') {
                    cx.clientdata.getAllAsync('dataItem', {
                    ...
                    cx.clientdata.getAllAsync('sourceData', {
                        code: vlist[0],
        ...
        datetime: {//日期框
            ...
            renderTable: function (compont, $row) {//使用表单的时候渲染成table
                ...
                datedefault = cx.formatDate(datedefault, dateformat.replace(/H/g, 'h'));
                ...
                function register() {
                    if ($('#' + compont.startTime).length > 0 && $('#' + compont.endTime).length > 0) {
                        $('#' + compont.startTime).on('change', function () {
                            ...
                            if (!!st && !!et) {
                                var diff = cx.parseDate(st).DateDiff('d', et) + 1;
                                $compont.val(diff);
                            }
                        });
        ...
        encode: {//单据编码
            ...
            property: function ($component) {
                ...
                if (!!ruleCode) {
                    ...
                    cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/CodeRule/GetList', {}, function (data) {
                        ruleCode = data;
                        $('#lr_component_rulecode').lrselectRefresh({
                            data: ruleCode
                        });
            ...
            renderTable: function (compont, $row) {//使用表单的时候渲染成table
                ...
                cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/CodeRule/GetEnCode', { code: compont.rulecode }, function (data) {
                    if (!$compont.val()) {
                        $compont.val(data);
                    }
                });
            ...
            renderTable: function (compont, $row) {
                ...
                switch (compont.dataType)
                {
                    ...
                    case 'user':
                        ...
                        break;
                    case 'time':
                        $compont[0].lrvalue = cx.formatDate(new Date(), 'yyyy-MM-dd hh:mm:ss');
        ...
        girdtable: {
            ...
            renderTable: function (compont, $row) {
                ...
                if (!!compont.preloadDb && !!compont.preloadTable) {
                    ...
                    cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetTableDataAllList', { databaseLinkId: compont.preloadDb, tableName: compont.preloadTable }, function (data) {
            ...
            select: function (item) {
                if (!!item) {
                    ...
                    if (!!fieldMap[designerDfop.dbId + item.name]) {
                        ...
                        cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: designerDfop.dbId, tableName: item.name }, function (data) {
    ...
    function toGirdheadData(data, hideData, $compont) {
        ...
        $.each(data, function (_index, item) {
            var isAdd = true;
            var point = { id: item.id, label: item.name, name: item.field || cx.newGuid(), width: parseInt(item.width || 100), align: item.align || "left" };
            switch (item.type) {
