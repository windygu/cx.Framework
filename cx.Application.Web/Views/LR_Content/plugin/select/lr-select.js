 ...
 * 创建人：力 软-前端开发组
 * 日 期：2018.03.22
 * 描 述：cxSelect（普通，多选，树形数据，gird，搜索，输入框选择器）-渲染数据在点击的时候触发，考虑到在一个表单上有超级多的下拉框的绑定情况（这里需要考虑赋值的特殊性）
 */
(function ($, cx) {
    "use strict";
    ...
    $.lrselect = {
        ...
        initRender: function (dfop, $self) {
            $('#cx_select_option_' + dfop.id).remove();
            var $option = $('<div class="lr-select-option" id="cx_select_option_' + dfop.id + '"></div>');

            var $optionContent = $('<div class="lr-select-option-content"></div>');
            var $ul = $('<ul id="cx_select_option_content' + dfop.id + '"></ul>');
        ...
        defaultRender: function (dfop) {
            var $ul = $('#cx_select_option_content' + dfop.id);
            $ul.html("");
            if (dfop.placeholder) {
            ...
            for (var i = 0, l = dfop.data.length; i < l; i++) {
                ...
                if (!item._lrhide) {
                    if (!!dfop.multipleMapValue && dfop.multipleMapValue[i] != undefined) {
                        var $li = $('<li data-value="' + i + '" class="lr-selectitem-li" ><img class="lr-select-node-cb" src="/Content/images/cxtree/checkbox_1.png">' + item[dfop.text] + '</li>');
                        $ul.append($li);
                    }
                    else {
                        var $li = $('<li data-value="' + i + '" class="lr-selectitem-li" ><img class="lr-select-node-cb" src="/Content/images/cxtree/checkbox_0.png">' + item[dfop.text] + '</li>');
                        $ul.append($li);
                    }
                }
            }
        ...
        treeRender: function (dfop) {
            var $option = $('#cx_select_option_' + dfop.id);
            ...
            if (dfop.type != 'multiple') {
                ...
                if ($et.hasClass('lr-selectitem-li') || $et.hasClass('lr-select-node-cb')) {
                    ...
                    if (_index == -1) {
                        ...
                        $option.find('.lr-select-node-cb[src$="checkbox_1.png"]').attr('src', '/Content/images/cxtree/checkbox_0.png');
                        $option.slideUp(150);
                        $this.removeClass('lr-select-focus');
                    }
                    else {
                        ...
                        if (dfop.multipleMapValue[_index] == undefined) {
                            ...
                            $cbobj.attr('src', '/Content/images/cxtree/checkbox_1.png');
                        }
                        else {
                            dfop.multipleValue = [];
                            dfop.multipleText = [];
                            ...
                            selected = false;
                            $cbobj.attr('src', '/Content/images/cxtree/checkbox_0.png');
                        }
    ...
    $.fn.lrselect = function (op) {
        ...
        // url优先级最高
        if (dfop.url) {
            cx.httpAsync(dfop.method, dfop.url, dfop.param, function (data) {
            ...
            if (dfop.isload == false) {
                ...
                switch (dfop.type) {
                    ...
                    case 'multiple':
                        ...
                        dfop.multipleText = [];
                        if (dfop.isrender) {
                            $this.find('.lr-select-node-cb[src$="checkbox_1.png"]').attr('src', '/Content/images/cxtree/checkbox_0.png');
                        }
                        ...
                        for (var i = 0, l = dfop.data.length; i < l; i++) {
                            ...
                            if (_arrayIndex != -1) {
                                ...
                                if (dfop.isrender) {
                                    $this.find('[data-value="' + i + '"] .lr-select-node-cb').attr('src', '/Content/images/cxtree/checkbox_1.png');
                                }
                                if (dfop.select) {
