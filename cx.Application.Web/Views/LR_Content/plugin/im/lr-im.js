...
(function ($, cx) {
    ...
    $.imServer = {
        ...
        , getUserInfo: function (callback) {
            userinfo = cx.clientdata.get(['userinfo']);
            if (!!userinfo) {
                callback();
            }
            else {
...
(function ($, cx) {
    ...
    $.lrIM = {
        ...
        addMsgTolist: function (id, name, msg, img) {
            ...
            if ($item.length > 0) {
                ...
                _html += '<img src="' + top.$.rootUrl + '/Content/images/cxim/' + img + '">';
                _html += '';
    ...
    $._lrIM = {
        render: function () {
            ...
            /*联系人列表*/
            _html += '<div class="lr-im-user-list" style="display:none;" id="cx_im_list" >';
            _html += '<div class="lr-im-header">企业内部通讯<div class="lr-im-close"><a id="cx_im_close" href="javascript:;">×</a></div></div>';
            ...
            _html += '<div class="lr-im-body-nav" id="cx_im_list_nav" ><ul>';
            ...
            _html += '<div class="lr-im-body-list" id="cx_im_body_list">';

            _html += '<div id="cx_im_last_list" class="cx_im_body_ul active" ></div>';
            _html += '<div id="cx_im_user_list" class="cx_im_body_user" style="display:none;" ><div class="lr-top-department" > <div id="im_department"></div> </div><div class="lr-userlist-content" id="lr_userlist_content" > <ul id="lr_userlist" class="lr-im-chatlist"></ul></div></div>';
            _html += '<div id="cx_im_group_list" class="cx_im_body_ul" style="display:none;" ></div>';
            ...
            /*聊天窗口*/
            _html += '<div class="lr-im-window" style="display:none;" id="cx_im_window">';
            ...
            // 关闭按钮
            $('#cx_im_close').on('click', function () {
                var $im = $('#cx_im_list');
                var $im_message_window = $('#cx_im_window');
                $im.hide();
                $im_message_window.hide();
            });

            // 联系人列表切换
            $('#cx_im_list_nav li').on('click', function () {
                var $this = $(this);
                if (!$this.hasClass('active')) {
                    ...
                    var id = '#cx_im_' + $this.attr('data-value') + '_list';
                    var $list = $(id);

                    $('#cx_im_body_list>div.active').removeClass('active').hide();
                    $list.addClass('active').show();
                }
            });
            // 打开聊天窗口
            $('#cx_im_body_list').delegate(".lr-user-item", "click", function (e) {
                var $this = $(this);
            ...
            // 发送消息
            var $textarea = $('#cx_im_window .lr-im-window-send');
            $textarea.delegate("textarea", 'keypress', function (e) {
                ...
                if (shiftKey && keyCode == "13") {
                    ...
                    if (sendText) {
                        $._lrIM.addRightMsg(userinfo.realName, cx.getDate('yyyy-MM-dd hh:mm'), $._lrIM.getUserImg(), sendText);
                    }
                    $('#cx_im_window .lr-im-window-send').html('<textarea autofocus placeholder="按回车发送消息,shift+回车换行"></textarea>');
                    setTimeout(function () {
                        $('#cx_im_window .lr-im-window-send>textarea').focus();
                    }, 100);
                    e.preventDefault();
                    return false;
                }
            ...
            $('#cx_im_window .lr-im-window-chat').mCustomScrollbar({ // 优化滚动条
                theme: "minimal-dark"
            });
            //$('#cx_im_body_list').mCustomScrollbar({ // 优化滚动条
            //    theme: "minimal-dark"
            //});
        }
        ...
        , getUserHtml: function (userItem) {
            if (userItem.F_UserId != userinfo.userId) {
                var _li = '';
                if (cx.isExistImg(top.$.rootUrl + userItem.F_HeadIcon)) {
        ...
        getUserImg: function () {
            var headimg = '';
            if (cx.isExistImg(top.$.rootUrl + userinfo.headIcon)) {
        ...
        addRightMsg: function (userName, datatime, img, msg) {
            ...
            $('#cx_im_window .lr-im-window-content').append(html);
            $("#cx_im_window .lr-im-window-chat").mCustomScrollbar("scrollTo", 'bottom');
        }
    }
})(jQuery, top.cx);
