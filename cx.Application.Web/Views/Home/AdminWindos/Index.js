...
var bootstrap = function ($, cx) {
    ...
    var meuns = {
        ...
        load: function () {
            var modulesTree = cx.clientdata.get(['modulesTree']);
            // 第一级菜单
            var parentId = '0';
            ...
            $('.lr-menu-item-text').each(function () {
                var $this = $(this);
                var text = $this.text();
                cx.language.get(text, function (_text) {
                    $this.text(_text);
            ...
            $('.icon-text').each(function () {
                var $this = $(this);
                var text = $this.text() || '';
                cx.language.get(text.replace(/^\s+|\s+$/g, ""), function (_text) {
                    $this.text(_text);
                });
            });
        },
        bind: function () {
            ...
            $('#lr_frame_menu a').on('click', function () {
                var $obj = $(this);
                var id = $obj.attr('id');
                var _module = cx.clientdata.get(['modulesMap', id]);
                switch (_module.F_Target) {
                    case 'iframe':// 窗口
                        meuns.startMenuClick();
                        setTimeout(function () {
                            if (cx.validator.isNotNull(_module.F_UrlAddress).code) {
                                cx.frameTab.open(_module);
                            }
                            else {
    ...
    cx.frameTab.opencallback = function () {
        meuns.closeApplist();
    };
    cx.frameTab.closecallback = function () {
    ...
    var getTime = function (time) {
        ...
        if (c <= 1) {
            return cx.formatDate(time, 'hh:mm:ss');
        }
        else {
            return cx.formatDate(time, 'yyyy/MM/dd');
        }
    }
    ...
    var im = {
        ...
        load: function () {
            // 获取下公司列表
            cx.clientdata.getAllAsync('company', {
                callback: function (data) {
                    ...
                    // 获取部门列表
                    cx.clientdata.getAllAsync('department', {
                        callback: function (data) {
                            $.each(data, function (_id, _item) {
                                _item.id = _id;
                                if (_item.parentId == "0") {
                            ...
                            // 获取人员数据
                            cx.clientdata.getAllAsync('user', {
                                callback: function (data) {
                                    $.each(data, function (_id, _item) {
                                        _item.id = _id;
                                    ...
                                    // 获取最近联系人列表
                                    cx.im.getContacts(function (data) {
        ...
        bind: function () {
            ...
            $('#lr_immsg_userlist .lr-scroll-box').on('click', function (e) {
                ...
                if ($et.length > 0) {
                    if (!$et.hasClass('active')) {
                        ...
                        // 获取聊天信息
                        cx.im.getMsgList(imUserId, function (data) {
                            var len = data.length;
                            if (len > 0) {
            ...
            $('#lr_im_input').on("keypress", function (e) {
                ...
                if (e.keyCode == "13") {
                    ...
                    if (text.replace(/(^\s*)|(\s*$)/g, "") != '') {
                        var time = cx.im.sendMsg(imUserId, text);
                        sendMsg(text, time);
                        var $userItem = $('#lr_immsg_userlist .lr-scroll-box [data-value="' + imUserId + '"]');
                        $userItem.find('.msg').text(text);
                        $userItem.find('.date').text(getTime(cx.getDate('yyyy-MM-dd hh:mm:ss')));
                        $userItem = null;
                    }
                    return false;
                }
            ...
            // 注册消息接收
            cx.im.registerRevMsg(function (userId, msg, dateTime) {
                var $userList = $('#lr_immsg_userlist .lr-scroll-box');
                var $userItem = $userList.find('[data-value="' + userId + '"]');
                // 判断当前账号是否打开聊天窗口
                if (userId == imUserId) {
                    revMsg(userId, msg, dateTime);
                    cx.im.updateContacts(userId);
