...
var bootstrap = function ($, cx) {
    ...
    var page = {
        ...
        /*绑定事件和初始化控件*/
        bind: function () {
            cx.httpAsyncGet(top.$.rootUrl + '/LR_SystemModule/Module/GetCheckTree', function (res) {
                if (res.code == 200) {
                    treeData = res.data;
                    setTimeout(function () {
                        $('#step-1').lrtree({
            ...
            $('#wizard').wizard().on('change', function (e, data) {
                ...
                if (data.direction == "next") {
                    if (data.step == 1) {
                        ...
                        $('#step-2 .lr-tree-root [id$="_cx_moduleId"]').parent().hide();
                        $('#step-3 .lr-tree-root [id$="_cx_moduleId"]').parent().hide();
                        $('#step-4 .lr-tree-root [id$="_cx_moduleId"]').parent().hide();
                        $.each(checkModuleIds, function (id, item) {
                            $('#step-2_' + item.replace(/-/g, '_') + '_cx_moduleId').parent().show();
                            $('#step-3_' + item.replace(/-/g, '_') + '_cx_moduleId').parent().show();
                            $('#step-4_' + item.replace(/-/g, '_') + '_cx_moduleId').parent().show();
                        });
                    } else if (data.step == 3) {
        ...
        save: function () {
            ...
            $.each(checkButtonIds, function (id, item) {
                if (item.indexOf('_cx_moduleId') == -1) {
