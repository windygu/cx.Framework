    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_delete').on('click', function () {
                var keyValue = $('#gridtable').jfGridValue('F_Id');
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认删除该功能！', function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/AppManager/FunctionManager/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_enable').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    if (enabledMark != 1) {
                        cx.layerConfirm('是否启用该功能！', function (res) {
                            if (res) {
                                cx.postForm(top.$.rootUrl + '/AppManager/FunctionManager/UpDateSate', { keyValue: keyValue, state: 1 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
            ...
            $('#lr_disabled').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    if (enabledMark == 1) {
                        cx.layerConfirm('是否禁用该功能！', function (res) {
                            if (res) {
                                cx.postForm(top.$.rootUrl + '/AppManager/FunctionManager/UpDateSate', { keyValue: keyValue, state: 0 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
