    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_delete').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/AppManager/DTImg/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_enabled').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    if (enabledMark != 1) {
                        cx.layerConfirm('是否确认启用该项！', function (res) {
                            if (res) {
                                cx.postForm(top.$.rootUrl + '/AppManager/DTImg/UpDateSate', { keyValue: keyValue, state: 1 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
            ...
            $('#lr_disabled').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    if (enabledMark == 1) {
                        cx.layerConfirm('是否确认禁用该项！', function (res) {
                            if (res) {
                                cx.postForm(top.$.rootUrl + '/AppManager/DTImg/UpDateSate', { keyValue: keyValue, state: 0 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
