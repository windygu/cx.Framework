...
var bootstrap = function ($, cx) {
    ...
    var page = {
        ...
        bind: function () {
            // 加载自定义流程列表
            cx.httpAsync('GET', top.$.rootUrl + '/LR_WorkFlowModule/WfScheme/GetCustmerSchemeInfoList', {}, function (data) {
                schemeList = data;
                render();
            });
    ...
    acceptClick = function () {
        ...
        if (!!shcemeCode) {
            ...
            // 发起流程
            cx.frameTab.open({ F_ModuleId: id, F_Icon: 'fa magic', F_FullName: '发起流程【' + $selected[0].shceme.F_Name + '】', F_UrlAddress:  '/LR_WorkFlowModule/WfMyTask/CustmerWorkFlowForm?shcemeCode=' + shcemeCode + '&tabIframeId=' + id + '&type=0' });
            return true;
        }
        else {
            cx.alert.error('请选择流程模板');
            return false;
        }
    };
    page.init();
