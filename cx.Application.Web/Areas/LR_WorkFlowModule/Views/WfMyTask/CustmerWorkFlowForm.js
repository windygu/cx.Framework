                                ...
                                if (flag) {
                                    ...
                                    cx.workflowapi.create({
                                        ...
                                        callback: function (res, data) {
                                            if (res) {
                                                cx.frameTab.parentIframe().refreshGirdData();
                                                cx.frameTab.close(tabIframeId);
                                            }                                            
                                        }
                                    });
                                }
                                else {
                                    cx.alert.error('表单数据保存失败');
                                }
                            }
                            else {
                                setTimeout(function () {
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#print').on('click', function () {
                ...
                var $iframe = cx.iframe(iframeId, frames);
        ...
        initflow: function () {
            switch (type) {
                case '0':
                    // 生成流程实例ID
                    processId = cx.newGuid();
                case '2':
                    cx.workflowapi.bootstraper({
                        ...
                        callback: function (res, data) {
                            if (res) {
                                ...
                                $('#confirm').on('click', function () {
                                    cx.layerConfirm('是否确认阅读！', function (res, index) {
                                        if (res) {
                                            cx.workflowapi.audit({
                                                taskId: taskId,
                                                verifyType: '6',
                ...
                case '100':
                    ...
                    break;
                case '101':
                    cx.workflowapi.processinfoByMonitor({
                        processId: processId,
                        taskId: taskId,
                        callback: function (res, data) {
                            if (res) {
        ...
        iframeLoad: function (iframeId, url, callback, i) {
            ...
            var _iframeLoaded = function () {
                var iframeObj = cx.iframe(iframeId, frames);
