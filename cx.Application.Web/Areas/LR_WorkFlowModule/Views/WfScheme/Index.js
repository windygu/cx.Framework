    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_delete').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_WorkFlowModule/WfScheme/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_enable').on('click', function () {
                ...
                if (type == 2) {
                    cx.alert.warning('草稿模板不能启用!');
                    return false;
                }

                if (cx.checkrow(keyValue)) {
                    if (enabledMark != 1) {
                        cx.layerConfirm('是否确认启用该表单模板！', function (res) {
                            if (res) {
                                cx.postForm(top.$.rootUrl + '/LR_WorkFlowModule/WfScheme/UpDateSate', { keyValue: keyValue, state: 1 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
            ...
            $('#lr_disabled').on('click', function () {
                ...
                var type = $('#gridtable').jfGridValue('F_Type');
                if (type == 2) {
                    cx.alert.warning('草稿模板不能禁用!');
                    return false;
                }

                if (cx.checkrow(keyValue)) {
                    if (enabledMark == 1) {
                        cx.layerConfirm('是否确认禁用该表单模板！', function (res) {
                            if (res) {
                                cx.postForm(top.$.rootUrl + '/LR_WorkFlowModule/WfScheme/UpDateSate', { keyValue: keyValue, state: 0 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
