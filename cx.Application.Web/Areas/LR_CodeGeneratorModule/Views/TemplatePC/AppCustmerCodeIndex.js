    ...
    var page = {
        ...
        bind: function () {
                    ...
                    if (data.step == 1) {
                        ...
                        $.each(dbTable, function (_index, _item) {
                            if (_item.relationName == '') {
                                ...
                                if (_item.field == '') {
                                    flag = false;
                                    cx.alert.error('表【' + _item.name + '】请设置关联字段！');
                                    return false;
                                }
                                if (_item.relationField == '') {
                                    flag = false;
                                    cx.alert.error('表【' + _item.relationField + '】请设置关联表对应字段！');
                                    return false;
                                }
                            }
                        });
                        ...
                        if (mapField[databaseLinkId + mainTable.name]) {
                            ...
                            if (!mapField[databaseLinkId + mainTable.name]) {
                                cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: databaseLinkId, tableName: mainTable.name }, function (data) {
                        ...
                        cx.httpAsyncPost(top.$.rootUrl + '/LR_CodeGeneratorModule/TemplatePC/LookAppCustmerCode', postData, function (res) {
                            if (res.code == 200) {
            ...
            $('#dbTree').lrtree({
                ...
                nodeClick: function (node) {
                    if (node.hasChildren) {
                        ...
                        // 获取数据的表数据 
                        cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetList', { databaseLinkId: databaseLinkId }, function (data) {
                            dbAllTable = data;
                            var tableData = [];
                ...
                onSelectRow: function (row, isCheck) {
                    if (isCheck) {
                        ...
                        if (!mapField[databaseLinkId + row.name]) {
                            cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: databaseLinkId, tableName: row.name }, function (data) {
                        ...
                        edit: {
                            ...
                            init: function (row, $self) {// 选中单元格后执行
                                if (mapField[databaseLinkId + row.relationName]) {
                                    ...
                                    cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: databaseLinkId, tableName: row.relationName }, function (data) {
            ...
            $('#treeDataSourceId').lrselect({
                ...
                select: function (item) {
                    if (!!item) {
                        cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DataSource/GetDataColName', { code: item.F_Code }, function (data) {
                            var fieldData = [];
            ...
            $('#lr_treesql_set').on('click', function () {
                ...
                cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetSqlColName', { databaseLinkId: databaseLinkId, strSql: strSql }, function (data) {
                    var fieldData = [];
                    for (var i = 0, l = data.length; i < l; i++) {
                        var id = data[i];
                        var selectpoint = { value: id, text: id };
            ...
            $('#appselectIcon').on('click', function () {
                cx.layerForm({
                    ...
                    maxmin: true,
                        ...
                        if (top._cxSelectIcon != '') {
                            $('#F_appIcon').val(top._cxSelectIcon);
                        }
                    }
                });
            });
