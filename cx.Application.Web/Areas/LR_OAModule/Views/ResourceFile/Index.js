/*
 * 版 本 cx-ADMS V6.1.6.0 力软敏捷开发框架(http://www.cx.cn)
    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr-download').on('click', function () {
                ...
                if (keyValue) {
                    if (fileType != 'folder') {
                        cx.download({ url: top.$.rootUrl + '/LR_OAModule/ResourceFile/DownloadFile', param: { keyValue: keyValue }, method: 'POST' });
                    } else {
                        cx.alert.error('目前不支持文件夹下载');
                    }
                } else {
                    cx.alert.warning('请选择要下载的文件！');
                }
            });
            //文件预览
            $('#lr-preview').on('click', function () {
                ...
                if (keyValue) {
                    if (fileType != 'folder') {
                        ...
                        cx.alert.error('请选择文件');
                    }
                } else {
                    cx.alert.warning('请选择要预览的文件！');
                }
            });
            //文件（夹）删除
            $('#lr-delete').on('click', function () {
                ...
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    cx.layerConfirm("注：您确定要删除此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？可在回收站还原！", function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/RemoveForm', { keyValue: keyValue, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                } else {
                    cx.alert.warning('请选择要删除的文件夹或文件！');
                }
            });
            //文件共享
            $('#lr-share').on('click', function () {
                ...
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    cx.layerConfirm("注：您确定要共享此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？", function (res) {
                        if (res) {
                            cx.warning(top.$.rootUrl + '/LR_OAModule/ResourceFile/ShareFile', { keyValue: keyValue, IsShare: 1, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                } else {
                    cx.alert.warning('请选择要共享的文件夹或文件！');
                }
            });
            //取消共享
            $('#lr-cancelshare').on('click', function () {
                ...
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    cx.layerConfirm("注：您确定要取消共享此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？", function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/ShareFile', { keyValue: keyValue, IsShare: 0, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                } else {
                    cx.alert.warning('请选择要取消共享的文件夹或文件！');
                }
            });
            //文件（夹）重命名
            $('#lr-rename').on('click', function () {
                ...
                if (keyValue) {
                    ...
                    cx.alert.warning('请选择要重命名的文件夹或文件！');
                }
            });
            //文件（夹）移动
            $('#lr-move').on('click', function () {
                ...
                if (keyValue) {
                    ...
                    cx.alert.warning('请选择要移动的文件夹或文件！');
                }
            });
            //文件（夹）还原
            $('#lr-restoreFile').on('click', function () {
                ...
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    cx.layerConfirm("注：您确定要还原此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？", function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/RestoreFile', { keyValue: keyValue, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                } else {
                    cx.alert.warning('请选择要还原的文件夹或文件！');
                }
            });
            //文件（夹）彻底删除
            $('#lr-thoroughDelete').on('click', function () {
                ...
                var fileType = $("#gridTable").jfGridValue("F_FileType");
                if (keyValue) {
                    cx.layerConfirm("注：您确定要删除此" + (fileType == "folder" ? "文件夹" : "文件") + "吗？该操作将无法恢复！", function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/ThoroughRemoveForm', { keyValue: keyValue, fileType: fileType }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr-emptyRecycled').on('click', function () {
                cx.layerConfirm("注：您确定要清空回收站吗？该操作将无法恢复！", function (res) {
                    if (res) {
                        cx.deleteForm(top.$.rootUrl + '/LR_OAModule/ResourceFile/EmptyRecycledForm', {}, function () {
                            refreshGirdData();
                        });
                    }
                });
