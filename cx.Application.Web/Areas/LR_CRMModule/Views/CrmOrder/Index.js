    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_add').on('click', function () {
                cx.frameTab.open({ F_ModuleId: 'order_add', F_Icon: 'fa fa-file-text-o', F_FullName: '新增订单', F_UrlAddress: '/LR_CRMModule/CrmOrder/Form' });
            });
            // 编辑
            $('#lr_edit').on('click', function () {
                var keyValue = $('#gridtable').jfGridValue('F_OrderId');
                if (cx.checkrow(keyValue)) {
                    cx.frameTab.open({ F_ModuleId: 'order_add', F_Icon: 'fa fa-file-text-o', F_FullName: '新增订单', F_UrlAddress: '/LR_CRMModule/CrmOrder/Form?keyValue=' + keyValue });
                }
            });
            // 删除
            $('#lr_delete').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_CRMModule/CrmOrder/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                ...
                headData: [
                    {
                        ...
                        formatter: function (cellvalue) {
                            return cx.formatDate(cellvalue, 'yyyy-MM-dd');
                        }
                    },
                    ...
                    { label: "单据编号", name: "F_OrderCode", width: 130, align: "left" },
                    {
                        ...
                        formatterAsync: function (callback, value, row) {
                            cx.clientdata.getAsync('custmerData', {
