...
var bootstrap = function ($, cx) {
    ...
    var page = {
        init: function () {
            // 获取自定义表单设置内容
            cx.httpAsync('GET', top.$.rootUrl + '/LR_FormModule/FormRelation/GetCustmerFormData', { keyValue: id }, function (data) {
        ...
        bind: function () {
            ...
            $('#lr_add').on('click', function () {
                if (settingJson.layer.opentype == '1') {// 窗口弹层页
                    ...
                    cx.frameTab.open({ F_ModuleId: id, F_Icon: 'fa fa-pencil-square-o', F_FullName: '新增', F_UrlAddress: '/LR_FormModule/Custmerform/TabInstanceForm?id=' + relation.F_FormId });
                }
            });
            // 编辑
            $('#lr_edit').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    if (settingJson.layer.opentype == '1') {// 窗口弹层页
                        ...
                        cx.frameTab.open({ F_ModuleId: id, F_Icon: 'fa fa-pencil-square-o', F_FullName: '编辑', F_UrlAddress: '/LR_FormModule/Custmerform/TabInstanceForm?id=' + relation.F_FormId + "&keyValue=" + keyValue });
                    }
                }
            });
            // 删除
            $('#lr_delete').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_FormModule/Custmerform/DeleteInstanceForm', { keyValue: keyValue, schemeInfoId: relation.F_FormId }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                    ...
                    switch (item.compont.type) {
                        ...
                        case 'select':
                            if (item.compont.dataSource == "0") {
                                ...
                                point.formatterAsync = function (callback, value, row) {
                                    cx.clientdata.getAsync('sourceData', {
                                        key: value,
                                        keyId: vlist[2],
                                        code: vlist[0],
                                        callback: function (_data) {
