    ...
    var page = {
        ...
        bind: function () {
                        ...
                        if (setFormId != formId) {
                            ...
                            $.lrSetForm(top.$.rootUrl + '/LR_FormModule/Custmerform/GetFormData?keyValue=' + formId, function (data) {//
                                ...
                                for (var i = 0, l = formscheme.data.length; i < l; i++) {
                                    ...
                                    for (var j = 0, jl = componts.length; j < jl; j++) {
                                        ...
                                        if (item.type != "gridtable" && item.type != "guid") {
                                            var point = { 'F_ModuleFormId': cx.newGuid(), 'F_EnCode': item.id, 'F_FullName': item.title };
                                ...
                                if (!!tableFields[formscheme.dbId + mainTable]) {
                                    ...
                                    cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldList', { databaseLinkId: formscheme.dbId, tableName: mainTable }, function (data) {
            ...
            $('#lr_delete_query').on('click', function () {
                var id = $('#query_girdtable').jfGridValue('id');
                if (cx.checkrow(id)) {
                    cx.layerConfirm('是否确认删除该字段', function (res, index) {
                        if (res) {
                            for (var i = 0, l = queryData.length; i < l; i++) {
        ...
        save: function () {
            ...
            for (var i = 0, l = colData.length; i < l; i++) {
                ...
                var point = { F_ModuleColumnId: cx.newGuid(), F_FullName: colData[i].fieldName, F_EnCode: code.toLowerCase() };
