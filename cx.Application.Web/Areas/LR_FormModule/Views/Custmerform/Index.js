            ...
            $('#lr_delete').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_FormModule/Custmerform/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                ...
                if (cx.checkrow(keyValue)) {
                    if (enabledMark != 1) {
                        cx.layerConfirm('是否确认启用该表单模板！', function (res) {
                            if (res) {
                                cx.postForm(top.$.rootUrl + '/LR_FormModule/Custmerform/UpDateSate', { keyValue: keyValue, state: 1 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
                ...
                if (cx.checkrow(keyValue)) {
                    if (enabledMark == 1) {
                        cx.layerConfirm('是否确认禁用该表单模板！', function (res) {
                            if (res) {
                                cx.postForm(top.$.rootUrl + '/LR_FormModule/Custmerform/UpDateSate', { keyValue: keyValue, state: 0 }, function () {
                                    refreshGirdData();
                                });
                            }
                        });
