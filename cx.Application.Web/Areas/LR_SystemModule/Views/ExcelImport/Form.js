            ...
            $('#lr_dbtree').lrtree({
                ...
                nodeClick: function (item) {
                    if (!item.hasChildren) {
                        dbTable = item.text;
                        dbId = item.value;
                        cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldTreeList', { databaseLinkId: item.value, tableName: item.text }, function (res) {
                            cols.length = 0;
                            $('#gridtable').jfGridSet('refreshdata', cols);
        ...
        initData: function () {
            ...
            if (!!keyValue) {
                $.lrSetForm(top.$.rootUrl + '/LR_SystemModule/ExcelImport/GetFormData?keyValue=' + keyValue, function (data) {//
                    ...
                    cx.httpAsync('GET', top.$.rootUrl + '/LR_SystemModule/DatabaseTable/GetFieldTreeList', { databaseLinkId: dbId, tableName: dbTable }, function (res) {
                        var map = {};
