...
var bootstrap = function ($, cx) {
    ...
    var mergeFileChunks = function (file) {
        ...
        param['ext'] = file.ext;
        param['templateId'] = id;
        cx.httpAsyncPost(top.$.rootUrl + "/LR_SystemModule/ExcelImport/ExecuteImportExcel", param, function (res) {
            var $fileItem = $('#lr_form_file_queue_list').find('#lr_filequeue_' + file.id);
            $fileItem.find('.lr-uploader-progress').remove();
            if (res.code == cx.httpCode.success) {
                if (res.data.Success != '0') {
                    cx.alert.success('导入成功' + res.data.Success + '条');
                }
                // 文件保存成功后
                ...
                if (res.data.Fail != '0')
                {
                    cx.download({ url: top.$.rootUrl + '/LR_SystemModule/ExcelImport/DownImportErrorFile', param: { fileId: fileInfo[file.id].fileGuid, fileName: fileInfo[file.id].name, __RequestVerificationToken: $.lrToken }, method: 'POST' });
                }
            }
            else {
    ...
    var reomveFileChunks = function (file) {
        ...
        param['chunks'] = fileInfo[file.id].chunks;
        cx.httpAsyncPost(top.$.rootUrl + "/LR_SystemModule/Annexes/MergeAnnexesFile", param, function (res) { });
        var $fileItem = $('#lr_form_file_queue_list').find('#lr_filequeue_' + file.id);
        $fileItem.find('.lr-uploader-progress').remove();
        $fileItem.append('<div class="lr-msg"><i class="fa fa-exclamation-circle"></i></div>');
    }
    ...
    var page = {
        ...
        init: function () {
            ...
            $('#lr_down_file_btn').on('click', function () {
                cx.download({ url: top.$.rootUrl + '/LR_SystemModule/ExcelImport/DownSchemeFile', param: { keyValue: id, __RequestVerificationToken: $.lrToken }, method: 'POST' });
            });
        ...
        fileQueued: function (file) {// 文件加载到队列
            ...
            $item.append('<span class="lr-file-name">' + file.name + '(' + cx.countFileSize(file.size) + ')</span>');
