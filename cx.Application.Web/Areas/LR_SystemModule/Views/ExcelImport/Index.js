    ...
    var page = {
        ...
        bind: function () {
            ...
            $('#lr_delete').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_SystemModule/ExcelImport/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_enable').on('click', function () {
                var keyValue = $('#gridtable').jfGridValue('F_Id');
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认要【启用】！', function (res) {
                        if (res) {
                            cx.postForm(top.$.rootUrl + '/LR_SystemModule/ExcelImport/UpdateForm?keyValue=' + keyValue, { F_EnabledMark: 1 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_disable').on('click', function () {
                var keyValue = $('#gridtable').jfGridValue('F_Id');
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认要【停用】！', function (res) {
                        if (res) {
                            cx.postForm(top.$.rootUrl + '/LR_SystemModule/ExcelImport/UpdateForm?keyValue=' + keyValue, { F_EnabledMark: 0 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
                ...
                headData: [
                    { label: "模板名称", name: "F_Name", width: 160, align: "left" },
                    {
                        ...
                        formatter: function (cellvalue) {
                            var data = cx.clientdata.get(['modulesMap']);
