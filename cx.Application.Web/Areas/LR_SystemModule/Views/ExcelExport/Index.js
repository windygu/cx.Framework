            ...
            $('#lr_delete').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认删除该项！', function (res) {
                        if (res) {
                            cx.deleteForm(top.$.rootUrl + '/LR_SystemModule/ExcelExport/DeleteForm', { keyValue: keyValue }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_enable').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认要【启用】！', function (res) {
                        if (res) {
                            cx.postForm(top.$.rootUrl + '/LR_SystemModule/ExcelExport/UpdateState', { keyValue: keyValue, state: 1 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
            ...
            $('#lr_disable').on('click', function () {
                ...
                if (cx.checkrow(keyValue)) {
                    cx.layerConfirm('是否确认要【停用】！', function (res) {
                        if (res) {
                            cx.postForm(top.$.rootUrl + '/LR_SystemModule/ExcelExport/UpdateState', { keyValue: keyValue, state: 0 }, function () {
                                refreshGirdData();
                            });
                        }
                    });
